...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (typeof email !== "string" || !Array.isArray(cartDetails)) {
      return NextResponse.json({ error: "Missing email or cartDetails" }, { status: 400 });
    }
    ...
    if (!templates.every((t) => t.vendor_id === vendorId)) {
      return NextResponse.json({ error: "Only one vendor allowed per checkout" }, { status: 400 });
    }
    ...
    // verify transfers capability
    try {
      const adminStripe = new Stripe(process.env.STRIPE_SECRET_KEY!, { apiVersion: "2025-05-28.basil" });
